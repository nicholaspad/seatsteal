...
class TestGetUserSubscriptionTierCaching:
    ...
    def test_cache_miss_queries_database_and_caches_result(
        self, test_db: Session, test_college
    ):
        """Test that cache miss queries database and caches the result."""
        with patch("utils.premium.get_cached_user_tier") as mock_get_cache, patch(
            "utils.premium.cache_user_tier"
        ) as mock_cache_tier:
    ...
    @pytest.mark.unit
    def test_cache_handles_multiple_subscriptions_returns_latest(
        self, test_db: Session, test_college
    ):
    ...
    @pytest.mark.unit
    def test_cache_failure_does_not_break_tier_lookup(
        self, test_db: Session, test_college
    ):
