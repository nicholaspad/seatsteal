...
class UPennScraper(BaseScraper):
    """
    University of Pennsylvania course scraper.

    Scrapes course data from UPenn's Path@Penn system using their FOSE API.
    Based on Brown scraper - UPenn uses the same FOSE API system.
    """

    BASE_URL = "https://courses.upenn.edu"
    COURSE_SEARCH_URL = (
        "https://courses.upenn.edu/api/?page=fose&route=search&is_ind_study=N&is_canc=N"
    )
    CLASS_DETAILS_URL = "https://courses.upenn.edu/api/?page=fose&route=details"

    def __init__(self, db_session=None):
        super().__init__("upenn")
        self.current_term = get_term_code_from_db(db_session, "upenn")
    ...
    async def scrape_courses(
        ...
        """
        Scrape UPenn courses for a specific department or all courses.
        ...
        """
        logger.info(
            f"Scraping UPenn {department} courses (limit: {limit}, term: {self.current_term})"
        )
        ...
        try:
            ...
            logger.error(f"Failed to scrape UPenn {department}: {e}")
            raise
        finally:
            if self.client:
    ...
    async def _fetch_all_courses(
        ...
        """
        Fetch all courses from UPenn's API.
        ...
        try:
            ...
            logger.info(f"Found {len(courses)} courses from UPenn API")

            if not courses:
                logger.warning("No courses found via UPenn API")
                return []
            ...
            logger.error(f"Error during UPenn API course fetch: {e}")
            raise
    ...
    async def _fetch_course_list(self) -> List[Dict[str, Any]]:
        """
        Fetch the list of all courses from UPenn's API.
    ...
    async def _fetch_course_classes(
        ...
        UPenn's search API includes all necessary class information,
        ...
        Returns:
            Course dictionary with classes (skips classes with unknown status)
        """
        try:
            classes = []
            ...
            if main_class:
                # Skip classes with unknown status
                if main_class.get("status") != "Unknown":
            ...
            # Only return course if it has at least one valid class
            if not classes:
                return None
    ...
    def _transform_course_to_class(
        ...
        UPenn's search API includes class information in the course object.
    ...
    def _map_status_code(self, status_code: str) -> str:
        """
        Map UPenn's status codes to standard enrollment status.

        Args:
            status_code: Status code from UPenn API
    ...
    def _deduplicate_courses(
        ...
        UPenn API returns separate entries for each section of a course,
    ...
    async def _make_api_request(
        ...
        """
        Make a POST request to UPenn's API with URL-encoded JSON body.
        ...
        try:
            ...
            # Encode the body as URL-encoded JSON string (matching FOSE API expectations)
