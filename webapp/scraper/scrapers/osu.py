...
class OsuScraper(BaseScraper):
    ...
    async def _fetch_by_catalog_shard(self, shard: int, max_pages: int = 50) -> List[Dict[str, Any]]:
        ...
        """
        # Hard limit to prevent 503 errors
        if max_pages > 50:
    ...
    async def _fetch_department_courses(
        ...
        all_courses = []
        page = 1
        max_pages = 50  # Hard limit to prevent 503 errors
    ...
    def _transform_section(self, section: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            # CRITICAL FIX: Use classNumber from API, NOT section code
            # The API provides classNumber (e.g., "5458") which is the unique identifier
            # The section field (e.g., "0010") is just the section code
            ...
            if enrollment_status.lower() == "open":
                ...
                status = "Closed"
                logger.debug(
                    f"Unknown enrollment status '{enrollment_status}' for classNumber {class_number}"
                )

            return {
                "class_number": str(class_number),
                "section": str(section_code),
                "status": status,
            }
