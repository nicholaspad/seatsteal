...
def scrape_college_task(
    self, college_short_name: str, department: str, limit: int = None
):
    """
    ...
    """
    logger.info(
        f"[Task {self.request.id}] Starting scrape: {college_short_name} {department}"
    )
...
def scrape_all_colleges_task(self, department: str = None):
    ...
    async def run_scrape_all():
        async with AsyncSessionLocal() as db:
            ...
            result = await db.execute(select(College).where(College.is_active == True))
                ...
                for dept in departments_to_scrape:
                    try:
                        ...
                        result = await service.scrape_college(
                            college.short_name, dept, limit=None
                        )
                        results.append(result)
                    except Exception as e:
                        logger.error(
                            f"Failed to scrape {college.short_name} {dept}: {e}"
                        )
                        results.append(
                            {
...
def scrape_college_all_departments_task(self, college_short_name: str):
    ...
    """
    logger.info(
        f"[Task {self.request.id}] Starting full scrape for {college_short_name}"
    )
    ...
    common_departments = [
        "CS",
        "CSCI",
        "COS",  # Computer Science variants
        "MATH",
        "MA",  # Math variants
        "ENG",
        "ENGL",  # English variants
        "PHYS",
        "PHY",  # Physics variants
        "CHEM",
        "CHM",  # Chemistry variants
        "BIO",
        "BIOL",  # Biology variants
        "ECON",
        "ECO",  # Economics variants
        "PSYCH",
        "PSY",  # Psychology variants
    ]
    ...
    async def run_scrape_departments():
        async with AsyncSessionLocal() as db:
            ...
            for dept in common_departments:
                try:
                    result = await service.scrape_college(
                        college_short_name, dept, limit=None
                    )
                    if result.get("success") and result.get("courses_saved", 0) > 0:
                        results.append(result)
                except Exception as e:
                    logger.debug(
                        f"Department {dept} not found or failed for {college_short_name}: {e}"
                    )
                    continue
    ...
    try:
        ...
        logger.info(
            f"[Task {self.request.id}] Full scrape completed for {college_short_name}"
        )
        return result
    except Exception as e:
        logger.error(
            f"[Task {self.request.id}] Full scrape failed for {college_short_name}: {e}"
        )
        raise
